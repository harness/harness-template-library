locals {
  ...
  auth_type_valid = (
    contains(local.valid_oci_auth_types, var.auth_type)
    ? null
    : "[Invalid] Chosen OCI authentication type '${var.auth_type}' is not supported. Valid types: ${join(", ", local.valid_oci_auth_types)}"
  )
